...
### Introduction
yOS is a hobby operating system kernel written entirely in x86 assembly (NASM). It boots from real mode, switches to 32-bit protected mode, and provides a command-line interface with basic file system, disk management, and simple GUI-like TUI applications. It was created as a learning project to understand low-level system programming.

### Features
- 32-bit protected mode with paging (2 MB identity mapping)
...
- Keyboard driver with shift support and scancode-to-ASCII conversion
...
- TUI (Text User Interface) applications:
  - File Manager (with copy/paste/delete/rename)
  - Notepad (with undo/redo and file I/O)
  - Calculator (integer arithmetic)
  - Calendar (perpetual calendar with RTC)
  - Device Manager (hardware info)
  - Disk Manager (partition and format)
  - Games (Guess Number, Tic-Tac-Toe)
- System commands: reboot, shutdown, time, date, memory info, etc.
...
| `cd <path>` | Change directory (absolute or relative, e.g., `cd /`, `cd A/dir`) |
| `write <text> -2 <file>` | Write text to a file (creates if not exists) |
...
| Command | Description |
|---------|-------------|
| `tui` | Enter the Text User Interface (menu-driven) |
...
1. **TUI's Disk Mgr problem** – Can't use it properly

###  License & Credits

Developed independently by **YuZhuohao** as a personal project.

- Version: `snapshot_0.50`
...
### 功能特性
- 32 位保护模式，支持分页（2 MB 恒等映射）
- FAT12 和 FAT32 文件系统（读写支持）
- MBR 分区表管理（支持最多 4 个分区）
- IDE PIO 磁盘驱动（主/从，主/次通道）
- 键盘驱动（支持 Shift 键，扫描码转 ASCII）
- 简单进程管理（8 个槽位，轮询调度）
- 内存位图分配器（2 MB 物理内存）
- 文本模式 VGA 控制台（80×25），支持颜色
- 内置命令行 Shell
- TUI 应用程序：
  - 文件管理器（支持复制/粘贴/删除/重命名）
  - 记事本（支持撤销/重做，文件读写）
  - 计算器（整数运算）
  - 日历（万年历，支持 RTC）
  - 磁盘管理器（分区和格式化）
  - 游戏（猜数字、井字棋）
- 系统命令：重启、关机、时间、日期、内存信息等
...
| `part list` | 列出当前磁盘的分区 |
| `part sel <n>` | 选择分区（1..4） |
| `part fm [-fs fat12|fat32]` | 格式化当前分区（默认 FAT12，目前FAT32有故障，无法使用） |
| `part del` | 删除当前分区 |
| `dl list` | 列出所有已挂载的盘符 |
...
|------|------|
| `ls` | 列出当前目录内容 |
| `cd <路径>` | 切换目录（绝对或相对，例如 `cd ..`，`cd A/dir`） |
| `write <文本> -2 <文件>` | 将文本写入文件（不存在则创建，存在则追加） |
| `read <文件>` | 显示文件内容 |
| `crdir <名称>` | 创建目录 |
| `dedir <名称>` | 删除目录（支持通配符 `*`） |
| `del <名称>` | 删除文件（支持通配符 `*`） |
